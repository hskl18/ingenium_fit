...
export const request = {
  ...
  requestInterceptors: [
    (url: any, options: { headers: any }) => {
      ...
      headers['lang'] = LocaleLanguage[getLocale()];
      ...
      return { url: 'http://47.239.182.229:81/prod-api' + url, options };
      // return { url, options };
    },
  ],
  responseInterceptors: [
    (response) => {
      ...
      if (data.code !== 200) {
        message.error(data.msg || 'Request error, please retry.');
        if (data.code === 401 || data.code === 700) {
          history.push(PageEnum.LOGIN);
          clearSessionToken();
        }
      }
