 ...
 * @param selectedRows
 */
const cancelAuthUserAll = async (roleId: string, selectedRows: API.System.User[], intl: any) => {
	const hide = message.loading(intl.formatMessage({
		id: 'system.role.auth.canceling',
		defaultMessage: '正在取消授权',
	}));
	...
	try {
		...
		if (resp.code === 200) {
			message.success(intl.formatMessage({
				id: 'system.role.auth.cancelSuccess',
				defaultMessage: '取消授权成功，即将刷新',
			}));
		} else {
		...
		message.error(intl.formatMessage({
			id: 'system.role.auth.cancelFailed',
			defaultMessage: '取消授权失败，请重试',
		}));
		return false;
	}
};

const cancelAuthUser = async (roleId: string, userId: number, intl: any) => {
	const hide = message.loading(intl.formatMessage({
		id: 'system.role.auth.canceling',
		defaultMessage: '正在取消授权',
	}));
				...
				<Button
					...
					onClick={async () => {
						...
						content: intl.formatMessage(
							{ id: 'system.role.auth.cancelConfirm', defaultMessage: '确认要取消该用户{userName}的角色授权吗？' },
						...
						onOk: async () => {
							const success = await cancelAuthUser(roleId, record.userId, intl);
								if (success) {
			...
			<div style={{ width: '100%', float: 'right' }}>
              ...
              danger
							...
							onClick={async () => {
								...
								async onOk() {
									const success = await cancelAuthUserAll(roleId, selectedRows, intl);
										if (success) {
											setSelectedRows([]);
			...
			<UserSelectorModal
				...
				onSubmit={(values: React.Key[]) => {
					...
					if (userIds === "") {
						...
						id: 'system.role.auth.selectUser',
						defaultMessage: '请选择要分配的用户',
					}));
						return;
					}
