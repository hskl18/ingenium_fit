 ...
 * @param fields
 */
const handleAdd = async (fields: API.System.Role, intl: any) => {
 ...
 * @param fields
 */
const handleUpdate = async (fields: API.System.Role, intl: any) => {
  const hide = message.loading(intl.formatMessage({
 ...
 * @param selectedRows
 */
const handleRemove = async (selectedRows: API.System.Role[], intl: any) => {
  const hide = message.loading(intl.formatMessage({
...
const handleRemoveOne = async (selectedRow: API.System.Role, intl: any) => {
  const hide = message.loading(intl.formatMessage({
    id: 'pages.common.deleting',
    defaultMessage: '正在删除',
  }));
  ...
  const showChangeStatusConfirm = (record: API.System.Role) => {
    ...
    confirm({
      title: intl.formatMessage(
        { id: 'system.role.statusConfirm', defaultMessage: '确认要{action}{roleName}角色吗？' },
        { action: text, roleName: record.roleName }
      ),
      onOk() {
    ...
    <PageContainer>
      ...
      <UpdateForm
        onSubmit={async (values) => {
          let success = false;
          if (values.roleId) {
            success = await handleUpdate({ ...values } as API.System.Role, intl);
          } else {
            success = await handleAdd({ ...values } as API.System.Role, intl);
          }
          if (success) {
