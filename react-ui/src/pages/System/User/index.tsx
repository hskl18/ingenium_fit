 ...
 * @param fields
 */
const handleAdd = async (fields: API.System.User, intl: any) => {
 ...
 * @param fields
 */
const handleUpdate = async (fields: API.System.User, intl: any) => {
  const hide = message.loading(intl.formatMessage({
 ...
 * @param selectedRows
 */
const handleRemove = async (selectedRows: API.System.User[], intl: any) => {
  const hide = message.loading(intl.formatMessage({
...
const handleRemoveOne = async (selectedRow: API.System.User, intl: any) => {
  const hide = message.loading(intl.formatMessage({
    id: 'pages.common.deleting',
    defaultMessage: '正在删除',
  }));
...
const UserTableList: React.FC = () => {
  ...
  const showChangeStatusConfirm = (record: API.System.User) => {
    let text = record.status === "1" 
      ? intl.formatMessage({ id: 'system.user.enable', defaultMessage: '启用' })
      : intl.formatMessage({ id: 'system.user.disable', defaultMessage: '停用' });
    const newStatus = record.status === '0' ? '1' : '0';
    confirm({
      title: intl.formatMessage(
        { id: 'system.user.statusConfirm', defaultMessage: '确认要{action}{userName}用户吗？' },
        { action: text, userName: record.userName }
      ),
      onOk() {
        ...
        return (
          <Switch
            ...
            checkedChildren={intl.formatMessage({
              id: 'system.user.status.normal',
              defaultMessage: '正常',
            })}
            unCheckedChildren={intl.formatMessage({
              id: 'system.user.status.disabled',
              defaultMessage: '停用',
            })}
            defaultChecked
    ...
    <PageContainer>
      ...
      <UpdateForm
        onSubmit={async (values) => {
          let success = false;
          if (values.userId) {
            success = await handleUpdate({ ...values } as API.System.User, intl);
          } else {
            success = await handleAdd({ ...values } as API.System.User, intl);
          }
          if (success) {
      ...
      <ResetPwd
        onSubmit={async (values: any) => {
          ...
          if (success) {
            ...
            setCurrentRow(undefined);
            message.success(intl.formatMessage({
              id: 'system.user.resetPasswordSuccess',
